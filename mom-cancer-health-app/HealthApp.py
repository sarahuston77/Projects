...
class HealthReminderApp(App):
    ...
    def _build_header(self):
        ...
        self.subtitle_label = Label(
            text="Daily wellness & health reminders",
            font_size=dp(14),
            color=(1, 1, 1, 0.85),
            halign="left",
            valign="middle",
    ...
    def _waiting_message(self):
        return (
            ...
            "Reminders appear automatically at your scheduled times. "
    ...
    def log_sleep(self, instance):
        self.show_popup("Log Sleep", "How was your sleep last night? (1 = poor, 10 = excellent)", self.store_sleep)

    def log_mood(self, instance):
        self.show_popup("Log Mood", "How are you feeling today? (1 = low, 10 = great)", self.evaluate_mood)
            ...
            if mood < 6:
                ...
                if sent:
                    self.reminder_message_label.text = (
                        f"Mood {mood} saved. A message was sent to your caregiver."
                    )
            elif mood < 8:
                self.reminder_message_label.text = "Mood saved. Consider calling your daughter — she'd love to hear from you."
            else:
                self.reminder_message_label.text = "Mood saved. Wonderful — keep enjoying your day!"
    ...
    def store_sleep(self, sleep_value):
        try:
            ...
            if sleep < 6:
                self.reminder_message_label.text = "Sleep saved. Call Sara — she might be able to help you rest better."
            elif sleep < 8:
                self.reminder_message_label.text = "Sleep saved. You might need more rest — consider your sleep routine."
            else:
                self.reminder_message_label.text = "Sleep saved. Great job keeping up healthy sleep habits!"
    ...
    def send_sms_via_email(self, carrier, message, phone_number=""):
        ...
        try:
            ...
            self.reminder_message_label.text = (
                f"Mood saved, but SMS alert failed: {e}"
            )
            return False
